...
def init_model(cfg):
    ...
    model = HRNetModel(width=18, pipeline_version = 's2', small=False, ocr_width=64, with_aux_output=True, use_leaky_relu=True,
...
def train(model, cfg, model_cfg):
    ...
    train_augmentator = Compose([
        ShiftScaleRotate(shift_limit=0.1, scale_limit=0.0, rotate_limit=(-45, 45), border_mode=2, p=0.75),
    #     UniformRandomResize(scale_range=(0.75, 1.25)),
        RandomResizedCrop(*crop_size, scale=(0.3,1.0), p=0.75),
        Resize(*crop_size),
        Flip(p=0.75),
        RandomRotate90(p=0.75),
        PadIfNeeded(min_height=crop_size[0], min_width=crop_size[1], border_mode=0),
    #     RandomCrop(*crop_size),
        RandomBrightnessContrast(brightness_limit=(-0.25, 0.25), contrast_limit=(-0.2, 0.4), p=0.75),
